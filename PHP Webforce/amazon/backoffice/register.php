...
if (!empty($_POST)) {
    if (!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['repeat_password'])) {
        if ($_POST['password'] == $_POST['repeat_password'] && strlen($_POST['password']) >= 8) {
            ...
            $passwordCrypt = md5($password); // On crypte le mot de passe

            $sqlSelect = "SELECT * FROM user WHERE email = '$email'";
            // $sqlSelect est une variable qui stocke une chaine de caractère ressemblant à une requette sql.
            $tableUser = mysqli_query($connexion, $sqlSelect);

            if (mysqli_num_rows($tableUser) == 0 && filter_var($email, FILTER_VALIDATE_EMAIL)) {


                // Si il y a 0 correspondance alors l'email est donc bien vierge et peut être créer.
                // SINON l'email est déjà existant.

                $date_register = date('Y-m-d H:i:s');
                $userStatut = 0;

                $sqlInsert = "INSERT INTO `user`(`firstname`, `lastname`, `email`, `password`, `date_register`, `user_statut`) VALUES ('$firstname','$lastname','$email','$passwordCrypt','$date_register','$userStatut')";
                ...
                if (mysqli_query($connexion, $sqlInsert)) {
                    ...
                    echo "Erreur lors de l'insertion de l'utilisateur : " . mysqli_error($connexion);
                }
            } else {
